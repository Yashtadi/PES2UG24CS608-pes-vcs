...
int head_update(const ObjectID *new_commit) {
    ...
    if (last_slash) {
        *last_slash = '\0';
        // Create directory recursively (simple approach for 2-level depth)
        mkdir(dir_path, 0755);
    }
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // Step 3: Try to read current HEAD as parent commit
    ObjectID parent_id;
    ...
    // Step 6: Update HEAD to point to this new commit
